...
usage() {
  cat <<'USAGE'
Self-heal Hermes Gizmo after a reboot or Hermes update.

Usage:
  scripts/self-heal-gizmo.sh [options]
...
Guardrails:
  ...
  - Does not repair unless Gizmo is enabled and the selector hook is missing.
...
unit_path() {
  printf '%s\n' "${XDG_CONFIG_HOME:-$HOME/.config}/systemd/user/hermes-gizmo-self-heal.service"
}
...
install_systemd_unit() {
  ...
  cat >"$(unit_path)" <<EOF
[Unit]
Description=Hermes Gizmo self-heal
Documentation=https://github.com/rzyns/hermes-gizmo
...
ExecStart=/usr/bin/env bash "$ROOT_DIR/scripts/self-heal-gizmo.sh" --hermes-bin "$HERMES_BIN" --hermes-home "$HERMES_HOME"

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable hermes-gizmo-self-heal.service
  echo "Installed: $(unit_path)"
  echo "Run now: systemctl --user start hermes-gizmo-self-heal.service"
}

uninstall_systemd_unit() {
  command -v systemctl >/dev/null 2>&1 || fail "systemctl is not available"
  systemctl --user disable --now hermes-gizmo-self-heal.service >/dev/null 2>&1 || true
  rm -f "$(unit_path)"
  systemctl --user daemon-reload
...
step "Checking Gizmo health"
echo "Hermes: $HERMES_BIN"
echo "Hermes home: $HERMES_HOME"

DOCTOR_JSON="$("$HERMES_BIN" gizmo doctor 2>/dev/null || true)"
...
case "$ACTION" in
  ok)
    echo "Gizmo is healthy; no repair needed."
    exit 0
    ;;
  repair)
    echo "Gizmo selector hook needs repair."
    ;;
  skip:*)
...
step "Repairing Gizmo"
bash "$ROOT_DIR/scripts/install-hermes-gizmo.sh" \
