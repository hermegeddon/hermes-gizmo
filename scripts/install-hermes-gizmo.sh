...
usage() {
  cat <<'USAGE'
Install or repair Hermes Gizmo for the local Hermes dashboard.

Usage:
  scripts/install-hermes-gizmo.sh [options]
...
activates. Gizmo detects Hermes' native bridge and will not double-slim
those requests. Gizmo still provides deterministic slimming when native
...
if [[ "$(readlink -f "$ROOT_DIR")" == "$(readlink -f "$TARGET_DIR" 2>/dev/null || printf '%s' "$TARGET_DIR")" ]]; then
  ...
  mkdir -p "$TMP_DIR/src"
  cp -R "$ROOT_DIR/src/hermes_gizmo" "$TMP_DIR/src/"
  mkdir -p "$TMP_DIR/scripts"
  cp "$ROOT_DIR/scripts/install-hermes-gizmo.sh" "$TMP_DIR/scripts/"
  cp "$ROOT_DIR/scripts/troubleshoot-hermes-gizmo.sh" "$TMP_DIR/scripts/"
  cp "$ROOT_DIR/scripts/self-heal-gizmo.sh" "$TMP_DIR/scripts/"
  cp "$ROOT_DIR/scripts/update-hermes-and-repair-gizmo.sh" "$TMP_DIR/scripts/"
  rm -rf "$TARGET_DIR"
...
core_hook_status() {
  "$HERMES_BIN" gizmo doctor 2>/dev/null | "$HERMES_PYTHON" -c '
import json, sys
try:
...
if bash "$ROOT_DIR/scripts/troubleshoot-hermes-gizmo.sh" --quick --hermes-bin "$HERMES_BIN" --hermes-home "$HERMES_HOME"; then
