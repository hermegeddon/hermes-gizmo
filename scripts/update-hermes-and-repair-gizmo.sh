...
usage() {
  cat <<'USAGE'
Update Hermes, then reinstall/repair Hermes Gizmo.

Usage:
  scripts/update-hermes-and-repair-gizmo.sh [options]

Options:
  --no-backup       Pass --no-backup to `hermes update`.
  --backup          Pass --backup to `hermes update`.
  --no-restart      Repair Gizmo without restarting Hermes services.
...
those requests. The update repair below preserves Gizmo's dashboard,
...
step "Repairing Gizmo after Hermes update"
...
step "Done"
echo "Hermes was updated and Gizmo was repaired. Check above for the doctor result."
