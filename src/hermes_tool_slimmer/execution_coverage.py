...
def decision_log_paths(home: str | Path) -> list[Path]:
    """Return existing default and profile-local Tool Slimmer decision logs."""

    root = Path(home).expanduser()
    paths: list[Path] = []
    default_log = root / "tool-slimmer" / "decisions.jsonl"
    ...
    if profiles_root.is_dir():
        for profile_home in sorted(child for child in profiles_root.iterdir() if child.is_dir()):
            log = profile_home / "tool-slimmer" / "decisions.jsonl"
            if log.is_file():
                paths.append(log)
    return paths
