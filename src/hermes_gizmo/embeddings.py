...
def _ensure_numpy() -> Any:
    if np is None:
        raise ImportError(
            "numpy is required for semantic_hybrid mode. Install with: pip install hermes-gizmo[semantic]"
        )
    return np
...
class EmbeddingCache:
    ...
    Layout under root (default ``~/.hermes/gizmo/semantic_cache``):
    ...
    * ``{cache_key}.json`` — metadata mapping tool names to row indices plus
    ...
    def __init__(self, root: Path | str | None = None) -> None:
        self.root = Path(root or hermes_home() / "gizmo" / "semantic_cache").expanduser()
