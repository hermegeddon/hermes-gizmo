...
from .config import GizmoConfig, config_path, hermes_home, load_config
...
def recommended_config(
    ...
    *,
    ...
    if "gizmo_request_full_tools" in tools:
        base_always.append("gizmo_request_full_tools")
    base: dict[str, Any] = {
        "enabled": True,
        "mode": "keyword",
        ...
        "profiles": {
            "telegram": {
                "top_k": 4,
                "always_include": _present(["memory", "gizmo_request_full_tools"], tools),
                "always_exclude": _present(TEXT_NOISE_TOOLS, tools),
            },
            "slack": {
                "top_k": 6,
                "always_include": _present(["memory", "read_file", "search_files", "gizmo_request_full_tools"], tools),
            ...
            "cron": {
                "top_k": 10,
                "always_include": _present(["terminal", "read_file", "search_files", "gizmo_request_full_tools"], tools),
            },
            "webhook": {
                "top_k": 8,
                "always_include": _present(["gizmo_request_full_tools"], tools),
            },
        },
    }
    return base
...
def analyze_config(
    ...
    *,
    ...
    checklist.append({"id": "config_valid", "label": "Config loads", "status": "pass", "message": "Gizmo config can be read."})
    checklist.append(
        {
            "id": "index_ready",
            "label": "Tool index",
    ...
    checklist.append(
        {
            ...
            "status": "pass" if not tools or "gizmo_request_full_tools" in tools else "warn",
            "message": "Fallback tool is available." if not tools or "gizmo_request_full_tools" in tools else "Fallback tool was not found in the current index.",
        }
    )
    checklist.append(
        {
    ...
    return {
        "ok": True,
        "status": setup_status,
        "summary": "Gizmo is safest when it uses the live Hermes schemas, keeps the full-tool fallback available, and applies narrower budgets only to entry points that need them.",
        "config": {
            "mode": cfg.mode,
            "top_k": cfg.top_k,
        ...
        "recommended_yaml": dump_yaml({"gizmo": recommendation}),
...
def backup_dir() -> Path:
    path = hermes_home() / "gizmo" / "backups"
...
def backup_config(path: str | Path | None = None) -> Path:
    ...
    if target.is_file():
        ...
        write_private_text(backup, "# No config file existed before Gizmo advisor apply.\n")
    return backup
...
def apply_recommended_config(
    ...
    *,
    ...
    if not isinstance(data, dict):
        data = {}
    cfg = load_config(target) if target.is_file() else GizmoConfig()
    store = IndexStore()
    index = store.load() or {}
