...
from .config import GizmoConfig, config_path, load_config
...
def analyze_config(cfg: GizmoConfig, summary: dict[str, object] | None = None, indexed_tools: int = 0) -> dict[str, object]:
...
def _package_info() -> dict[str, object]:
    ...
    try:
        version = md.version("hermes-gizmo")
    except md.PackageNotFoundError:
        version = "unknown"
    return {"version": version, "module": spec.origin if spec else None}


def eval_prompts(cfg: GizmoConfig, schemas: list[dict[str, Any]], prompts: list[dict[str, Any]]) -> dict[str, object]:
    rows = []
    hits = 0
    fail_open_count = 0
...
def run_doctor(
    ...
    if target.is_file():
        try:
            ...
            enabled_status = "pass" if "gizmo" in enabled else "warn"
    ...
    checks["plugin_enabled"] = _check(
        enabled_status,
        "gizmo is listed in plugins.enabled"
        if enabled_status == "pass"
        else "gizmo is not listed in plugins.enabled",
        enabled_detail,
    )
    ...
    checks["native_hermes_tool_search"] = _check(
        ...
        "Hermes native Tool Search is available; Gizmo skips active slimming when bridge tools are already assembled"
    ...
    try:
        ...
        checks["core_selector_hook"] = _check(
            ...
            else "Hermes core does not advertise select_tool_schemas; rerun scripts/install-hermes-gizmo.sh to apply the local compatibility patch",
        )
    except Exception:
        checks["core_selector_hook"] = _check(
            "warn",
...
def setup_argparse(parser: argparse.ArgumentParser) -> None:
    ...
    coverage = sub.add_parser("execution-coverage", help="Report Gizmo decision-log coverage across profiles and Kanban worker runs")
...
def handle_cli(args: argparse.Namespace) -> int:
    ...
    if args.command == "recommend-config":
        print(dump_yaml({"gizmo": current_advisor().get("recommended_config", {})}))
        return 0
