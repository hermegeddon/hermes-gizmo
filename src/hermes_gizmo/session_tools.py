...
from .config import GizmoConfig, hermes_home, load_config
...
class SessionLoadedState:
    ...
    def __init__(
        ...
        self.state_path = (
            Path(path or hermes_home() / "gizmo" / "session_loaded.json")
            .expanduser()
        )
...
def _schema_is_eligible(schema: Schema, cfg: GizmoConfig) -> bool:
...
def _is_disabled_or_excluded(name: str, cfg: GizmoConfig) -> bool:
...
def gizmo_tool_search(args: dict, **kwargs: Any) -> str:
    ...
    try:
        ...
        if not schemas:
            return _json({
                ...
                "message": "Provide schemas, run inside Hermes with live tool definitions, or rebuild the Gizmo index.",
            })
...
def gizmo_tool_details(args: dict, **kwargs: Any) -> str:
...
def gizmo_loaded_tools(args: dict, **kwargs: Any) -> str:
