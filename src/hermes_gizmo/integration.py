...
def _load_config_for_hook() -> GizmoConfig:
    try:
        return load_config(strict=True)
    except Exception as exc:
        LOG.warning("gizmo config load failed; disabling selector for this request: %s", exc)
        return GizmoConfig(enabled=False)
...
def _sync_live_index(schemas: list[Schema], min_total_tools: int, context: dict[str, Any] | None = None) -> None:
    ...
    try:
        ...
        LOG.warning("gizmo live index sync failed: %s", exc)
...
def _ensure_recovery_tool_schemas(schemas: list[Schema]) -> tuple[list[Schema], list[str]]:
    """Append Gizmo recovery schemas when upstream toolset filtering hid them.

    ACP and other restricted toolset contexts can invoke this selector hook with
    a catalog that excludes plugin-registered Gizmo tools. Without these
...
def _always_include_schemas(schemas_by_name: dict[str, Schema], cfg: GizmoConfig) -> tuple[list[Schema], list[str]]:
    selected: list[Schema] = []
    names: list[str] = []
    for name in [*cfg.always_include, "gizmo_request_full_tools", HYDRATE_TOOL_NAME]:
    ...
    **kwargs: Any,
    ...
    try:
        ...
        if _full_tools_requested(conversation_history):
            ...
            if cfg.log_decisions:
                LOG.info("gizmo full schema fallback", extra={"gizmo": metrics})
                try:
                    record_decision(metrics, decision_context)
                except Exception as exc:
                    LOG.warning("gizmo decision logging failed: %s", exc)
        ...
        if len(schemas) < cfg.min_total_tools:
            ...
            if cfg.log_decisions:
                LOG.info("gizmo skipped", extra={"gizmo": metrics})
                try:
                    record_decision(metrics, decision_context)
                except Exception as exc:
                    LOG.warning("gizmo decision logging failed: %s", exc)
        ...
        if cfg.mode == "two_pass":
            ...
            if two_pass_fallback == "missing_hydrate_tool" and cfg.two_pass.fallback_to_keyword:
                fallback_cfg = GizmoConfig.from_mapping(
                ...
                result = SelectionResult(
                    ...
                    always_included=[
                        name for name in [*cfg.always_include, "gizmo_request_full_tools", HYDRATE_TOOL_NAME]
        ...
        if cfg.log_decisions:
            LOG.info("gizmo selection", extra={"gizmo": metrics})
            try:
                record_decision(metrics, decision_context)
            except Exception as exc:
                LOG.warning("gizmo decision logging failed: %s", exc)
...
def pre_llm_diagnostic_hook(**kwargs: Any) -> dict[str, str] | None:
    ...
    return {
        "context": (
            ...
            "Hermes Gizmo dry-run is enabled; "
            "schema selection is diagnostic-only for this turn."
        )
    }


SESSION_BRIDGE_TOOL_DETAILS_NAMES = {"gizmo_tool_details"}
SESSION_BRIDGE_LOADED_TOOLS_NAMES = {"gizmo_loaded_tools"}
...
def _sync_session_loaded_from_tool_result(
    *,
    ...
    call itself. This hook mirrors successful load/unload actions into the real
    session so the next select_tool_schemas hook can inject loaded tools.
    """
    ...
    try:
        ...
        LOG.debug("gizmo session bridge config load failed: %s", exc)
        return
...
def transform_loaded_tools_session_bridge_hook(**kwargs: Any) -> str | None:
    ...
    if not session_id:
        return None
    from .session_tools import gizmo_loaded_tools

    args_raw = kwargs.get("args")
    args = args_raw if isinstance(args_raw, dict) else {}
    bridged_args = {**args, "session_id": session_id}
    return gizmo_loaded_tools(bridged_args)


def _inject_session_loaded(selected: list[Schema], full_schemas: list[Schema], cfg: GizmoConfig, session_id: str | None = None) -> list[Schema]:
...
def maybe_register_selector_hook(ctx: Any) -> bool:
    ...
    if callable(register_hook):
        if valid_hooks is not None and "select_tool_schemas" not in valid_hooks:
            LOG.warning("Hermes selector hook is unavailable; gizmo will run diagnostics only")
            return False
        try:
