...
def _make_embedding_provider(config: GizmoConfig) -> Any:
...
class ToolSelector:
    def __init__(self, config: GizmoConfig | None = None) -> None:
    ...
    def select(self, user_message: str, schemas: list[Schema], **kwargs: object) -> SelectionResult:
        ...
        if isinstance(mode, str) and mode != self.config.mode:
            ...
            override["mode"] = mode
            return ToolSelector(GizmoConfig.from_mapping(override)).select(user_message, schemas)
...
def select_schemas(user_message: str, schemas: list[Schema], config: GizmoConfig | None = None, **kwargs: object) -> list[Schema]:
