...
def test_selector_always_includes_core_tools():
    cfg = GizmoConfig(top_k=3, always_include=["terminal", "read_file"])
...
def test_selector_respects_top_k_after_always_includes():
    cfg = GizmoConfig(top_k=2, always_include=["terminal"])
    result = ToolSelector(cfg).select("github code search", SCHEMAS)
...
def test_selector_always_includes_full_tools_request_when_available():
    ...
    schemas = [
        {"name": "gizmo_request_full_tools", "toolset": "gizmo", "description": "Request full tools"},
        {"name": "search_files", "toolset": "native", "description": "Search files in repo"},
        {"name": "terminal", "toolset": "native", "description": "Run shell commands"},
    ]
    result = ToolSelector(cfg).select("search files", schemas)
    assert result.selected_names == ["gizmo_request_full_tools", "search_files"]
    assert result.always_included == ["gizmo_request_full_tools"]
    assert "gizmo_request_full_tools" not in result.scores


def test_selector_always_includes_progressive_discovery_tools_when_available():
    cfg = GizmoConfig(top_k=0, always_include=[])
    schemas = [
        {"name": "gizmo_request_full_tools", "toolset": "gizmo", "description": "Request full tools"},
        {"name": "gizmo_tool_search", "toolset": "gizmo", "description": "Search available tools"},
        {"name": "gizmo_tool_details", "toolset": "gizmo", "description": "Get tool details and load it"},
        {"name": "gizmo_loaded_tools", "toolset": "gizmo", "description": "List session-loaded tools"},
...
def test_gizmo_introspection_tools_do_not_compete_for_rank():
    cfg = GizmoConfig(top_k=1, always_include=[])
    schemas = [
        {"name": "gizmo_select", "toolset": "gizmo", "description": "Select relevant tools for a query"},
        {"name": "gizmo_request_full_tools", "toolset": "gizmo", "description": "Request full tools"},
        {"name": "execute_code", "description": "Query a database by running code"},
    ]
    result = ToolSelector(cfg).select("query a database", schemas)
    assert result.selected_names == ["gizmo_request_full_tools", "execute_code"]
    assert "gizmo_select" not in result.selected_names
    assert "gizmo_select" not in result.scores
...
def test_selector_does_not_select_disabled_tools():
    cfg = GizmoConfig(top_k=5, always_include=[], disabled_toolsets=["github"])
    result = ToolSelector(cfg).select("github code search", SCHEMAS)
...
def test_selector_skips_non_dict_schemas(caplog):
    cfg = GizmoConfig(top_k=1, always_include=[])
    with caplog.at_level("WARNING", logger="hermes_gizmo.selector"):
...
def test_selector_fails_open_on_index_error(monkeypatch):
    cfg = GizmoConfig(top_k=2, always_include=[])
...
def test_top_k_zero_does_not_fail_open_to_all_tools():
    cfg = GizmoConfig(top_k=0, always_include=[], fail_open=True)
    result = ToolSelector(cfg).select("search", SCHEMAS)
...
def test_no_match_keeps_always_include_only():
    cfg = GizmoConfig(top_k=3, always_include=["terminal"])
...
def test_weak_positive_matches_do_not_fill_top_k():
    cfg = GizmoConfig(top_k=3, always_include=["terminal"], min_score=99.0)
    result = ToolSelector(cfg).select("search", SCHEMAS)
...
def test_low_information_greeting_keeps_only_always_include_and_safety():
    cfg = GizmoConfig(top_k=4, always_include=["memory"])
    schemas = [
        {"name": "memory", "description": "Remember information"},
        {"name": "gizmo_request_full_tools", "description": "Request full tools"},
    ...
    assert result.selected_names == ["memory", "gizmo_request_full_tools"]
...
def test_selector_mode_kwarg_overrides_config():
    cfg = GizmoConfig(mode="keyword", top_k=1, always_include=[])
    schemas = [
...
def test_selector_validates_direct_config_instances():
    with pytest.raises(ValueError, match="top_k"):
        ToolSelector(GizmoConfig(top_k=-1))


def test_keyword_synonyms_route_browse_to_browser_navigation():
    cfg = GizmoConfig(top_k=3, always_include=[])
    schemas = [
...
def test_configured_aliases_expand_keyword_matching():
    cfg = GizmoConfig(top_k=1, always_include=[], aliases={"repo": ["github", "code"]})
...
def test_hybrid_mode_adds_fuzzy_token_boost():
    cfg = GizmoConfig(mode="hybrid", top_k=1, always_include=[])
    schemas = [
        {"name": "repository_lookup", "description": "Find repository metadata"},
        {"name": "slack_send_message", "description": "Send Slack message"},
    ]
