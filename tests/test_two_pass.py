...
from hermes_gizmo.integration import _HYDRATED_BY_SESSION, build_decision_context, select_tool_schemas_callback
from hermes_gizmo.tools import gizmo_hydrate_tools
from hermes_gizmo.two_pass import (
...
SCHEMAS = [
    ...
    {"name": "gizmo_request_full_tools", "description": "Request every tool"},
...
def test_selector_decision_log_includes_kanban_context(monkeypatch, tmp_path):
    ...
    event = json.loads(
        (tmp_path / "profiles" / "platform-eng" / "gizmo" / "decisions.jsonl")
        .read_text()
        .splitlines()[-1]
    )
...
def test_compact_catalog_is_deterministic_and_excludes_safety_tools():
    ...
    assert "gizmo_request_full_tools" not in first


def test_compact_catalog_respects_disabled_policy():
    cfg = GizmoConfig.from_mapping({"mode": "two_pass", "disabled_tools": ["web_search"]})
...
def test_hydrate_tool_handler_batches_requested_tools():
    result = json.loads(gizmo_hydrate_tools({"tools": ["web_search", "read_file", "web_search"], "reason": "need web"}))
...
def test_two_pass_first_pass_selects_only_always_and_hydrator(monkeypatch, tmp_path):
    ...
    cfg = GizmoConfig.from_mapping({"mode": "two_pass", "always_include": ["memory"]})
...
def test_two_pass_hydration_respects_disabled_policy(monkeypatch, tmp_path):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    _HYDRATED_BY_SESSION.clear()
    cfg = GizmoConfig.from_mapping({"mode": "two_pass", "always_include": ["memory"], "disabled_tools": ["web_search"]})
    marker = json.dumps({HYDRATE_REQUEST_MARKER: True, "tools": ["web_search", "read_file"]})
...
def test_two_pass_missing_hydrator_falls_back_to_keyword(monkeypatch, tmp_path):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    cfg = GizmoConfig.from_mapping({"mode": "two_pass", "always_include": [], "min_estimated_reduction_percent": 0})
